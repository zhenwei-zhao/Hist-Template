...
\usepackage[none]{hyphenat}      % 关闭自动断词

\hyphenpenalty=10000             % 禁止普通英文单词断开
\exhyphenpenalty=10000           % 禁止在已有连字符处断开
\emergencystretch=2em            % 给排版留一点弹性，减少文字溢出
...
\fancypagestyle{plain}{
  \fancyhf{}
  % \fancyhead[C]{\zihao{5}\song\leftmark}  居中的
  % \fancyfoot[C]{}

  \fancyhead[CE]{\song\zihao{5}\HISTTitleCN}
  \fancyhead[CE]{\song\zihao{5}\HISTHeader}
  \fancyhead[CO]{\song\zihao{5}\leftmark}
...
\fancypagestyle{frontstyle}{
  \fancyhf{}
  \fancyfoot[LE,RO]{\song\zihao{5}\thepage} % 奇数在右面，偶数在左面
  % \fancyfoot[C]{\song\zihao{5}\thepage} % 摘要页码居中；
...
\renewcommand{\chaptermark}[1]{\markboth{#1}{}}  % 是否显示第几章  注释掉显示
